...
var CalciteCombobox = /** @class */ (function () {
    function CalciteCombobox(hostRef) {
        ...
        this.inputHandler = function (event) {
            ...
            _this.filterItems(target.value);
        };
        ...
        this.comboboxFocusHandler = function (event) {
            _this.active = event.type === "focusin";
        };
    ...
    //
    // --------------------------------------------------------------------------
    CalciteCombobox.prototype.connectedCallback = function () {
        // prop validations
        var scale = ["s", "m", "l"];
        if (!scale.includes(this.scale))
            this.scale = "m";
    };
    CalciteCombobox.prototype.componentWillLoad = function () {
        this.updateItems();
    };
    CalciteCombobox.prototype.componentDidLoad = function () {
        this.observer.observe(this.el, { childList: true, subtree: true });
    };
    CalciteCombobox.prototype.componentDidUnload = function () {
        this.observer.disconnect();
    };
    CalciteCombobox.prototype.calciteComboboxItemChangeHandler = function (event) {
        this.toggleSelection(event.detail);
    };
    CalciteCombobox.prototype.calciteChipDismissHandler = function (event) {
        var _a;
    ...
    CalciteCombobox.prototype.handleInputKeyDown = function (event) {
    ...
    CalciteCombobox.prototype.toggleSelection = function (item, value) {
        if (value === void 0) { value = !item.selected; }
        item.selected = value;
        this.selectedItems = this.getSelectedItems();
        this.calciteLookupChange.emit(this.selectedItems);
    };
    CalciteCombobox.prototype.getVisibleItems = function () {
        return this.items.filter(function (item) { return !item.hidden; });
    };
    CalciteCombobox.prototype.getSelectedItems = function () {
        return this.items.filter(function (item) { return item.selected; });
    };
    CalciteCombobox.prototype.updateItems = function () {
        this.items = this.getItems();
        this.data = this.getData();
        this.selectedItems = this.getSelectedItems();
        this.visibleItems = this.getVisibleItems();
    };
    CalciteCombobox.prototype.getData = function () {
        return this.items.map(function (item) { return ({
            value: item.value,
            label: item.textLabel,
        }); });
    };
    CalciteCombobox.prototype.getItems = function () {
    ...
    CalciteCombobox.prototype.calciteComboboxItemKeyEventHandler = function (event) {
        var _b = event.detail, item = _b.item, keyboardEvent = _b.event;
        var isFirstItem = this.itemIndex(item) === 0;
        var isLastItem = this.itemIndex(item) === this.items.length - 1;
        var shiftKey = keyboardEvent.shiftKey;
        var keyCode = getKey(keyboardEvent.key);
        switch (keyCode) {
            case "Tab":
    ...
    CalciteCombobox.prototype.closeCalciteCombobox = function () {
        this.textInput.focus();
        this.active = false;
    };
    CalciteCombobox.prototype.focusFirstItem = function () {
        var firstItem = this.items[0];
        firstItem.focus();
    };
    CalciteCombobox.prototype.focusLastItem = function () {
        var lastItem = this.items[this.items.length - 1];
        lastItem.focus();
    };
    CalciteCombobox.prototype.focusNextItem = function (item) {
        var index = this.itemIndex(item);
        var nextItem = this.items[index + 1] || this.items[0];
        nextItem.focus();
    };
    CalciteCombobox.prototype.focusPrevItem = function (item) {
        var index = this.itemIndex(item);
        var prevItem = this.items[index - 1] || this.items[this.items.length - 1];
        prevItem.focus();
    };
    CalciteCombobox.prototype.itemIndex = function (item) {
    ...
    //
    //--------------------------------------------------------------------------
    CalciteCombobox.prototype.render = function () {
        var _this = this;
        var dir = getElementDir(this.el);
        var listBoxId = "listbox";
        return (h(Host, { active: this.active, onFocusin: this.comboboxFocusHandler, onFocusout: this.comboboxFocusHandler, dir: dir }, h("div", { class: "selections" }, this.selectedItems.map(function (item) {
            return (h("calcite-chip", { key: item.value, scale: _this.scale, value: item.value, dir: dir, dismissible: true }, item.textLabel));
        })), h("div", { role: "combobox", "aria-expanded": this.active.toString(), "aria-owns": listBoxId, "aria-haspopup": "listbox" }, h("input", { type: "text", placeholder: this.placeholder, "aria-label": this.label, "aria-autocomplete": "list", "aria-controls": listBoxId, onInput: this.inputHandler, disabled: this.disabled, onKeyDown: function (e) { return _this.handleInputKeyDown(e); }, ref: function (el) { return (_this.textInput = el); } })), h("ul", { id: listBoxId, role: "listbox", class: { list: true }, "aria-multiselectable": "true" }, h("slot", null))));
    };
    Object.defineProperty(CalciteCombobox.prototype, "el", {
