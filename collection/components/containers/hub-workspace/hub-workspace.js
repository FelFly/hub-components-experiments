        ...
        if (this.session !== undefined) {
            output.push(h("div", { class: "workspace-grid" },
                ...
                h("div", { class: "workspace-teams" },
                    ...
                    h("div", { class: "gallery" }, this.content.results.slice(0, 4).map((result) => h("hub-card", { class: "gallery-card", contenttype: `${HubTypes.HubType[result.hubType]} by ${result.publisher.name}`, image: result.thumbnailUrl, name: truncateString(result.name, 55), description: truncateString(result.summary, 90), url: result.url, buttonText: `View ${HubTypes.HubType[result.hubType]}`, onClick: () => "" }))),
